...
#
d <- list()
for (i in c("exp1t1","exp1t2","exp1t3")) {
  d[i] <- 1 - nrow(agg(experiment_name=i, number_of_runs=1)$raw)/100
}
failure_data_e1t1 <- data.frame(index=c(1,2,3,4,5), failure_percentage=unlist(d))

d <- list()
d[1] <- 1 - nrow(agg(experiment_name="exp1t1", number_of_runs=3)$raw)/100
...
#
# BoxPlot of trial 3 - Average execution time of each component service
#
services_time <- read.csv(file="factories-exp1t3_1execution1.csv.csv")
