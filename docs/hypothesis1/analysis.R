...
#################################################################
  ...
  d250 <- agg(experiment_name=experiment_name, number_of_runs=1)
  d250.title <- title
  ...
  d250.avg_composition_time_plot <- ggplot(data=d250_ordered, aes(x=start_time_millis, y=REAL_TIME_TO_SERVE_COMPOSITION/1000, colour=factorNbServices)) +
      geom_point() +
      geom_smooth() +
  ...
  d250.faceted_hist_compositions_plot <- ggplot(d250_merged) + 
    geom_histogram(aes(x=d250_merged$norm_real_time, y=..density..), binwidth=0.5) + 
...
############################################################
#
# Box plot showing the composition time means, per size of the network
#
############################################################
experiment_names <- c("data/h1_250_1", "data/h1_1k_1", "data/h1_4k_1", "data/h1_16k_1", "data/h1_64k_1")

experiment_results <- data.frame()

for ( experiment in experiment_names ) {
  d250_raw <- agg(experiment_name=experiment, number_of_runs=1)$raw
  d250_ordered <- relativeTime(d250_raw[order(d250_raw[,c("start_time_millis")]),])
  factorNbServices <- factor(unlist(lapply(d250_ordered$SERVICES_ENGAGED, nbServicesEngaged)))
  d250_ordered <- cbind(d250_ordered, composition_size=factorNbServices, experiment=experiment)
  d250_c2 <- d250_ordered[d250_ordered$composition_size == 2,]
  d250_c3 <- d250_ordered[d250_ordered$composition_size == 3,]
  d250_c2 <- cbind(d250_c2, norm_real_time=scale(d250_c2$REAL_TIME_TO_SERVE_COMPOSITION))
  d250_c3 <- cbind(d250_c3, norm_real_time=scale(d250_c3$REAL_TIME_TO_SERVE_COMPOSITION))
  
  experiment_results <- rbind(experiment_results, d250_c2)
  experiment_results <- rbind(experiment_results, d250_c3)
}
h1_composition_time_plot.labels <- c("250", "1k", "4k", "16k", "64k")

h1_composition_time_plot <- ggplot(experiment_results) + geom_boxplot(aes(x=experiment, y=REAL_TIME_TO_SERVE_COMPOSITION/1000)) +
  facet_grid(. ~ composition_size) +
...
#Zoomed in
ylim1 <- boxplot.stats(experiment_results$REAL_TIME_TO_SERVE_COMPOSITION/1000)$stats[c(1, 5)]
h1_composition_time_plot_zoom <- h1_composition_time_plot + coord_cartesian(ylim = ylim1*1.05)

ggsave(filename="plots/h1_average_composition_time.pdf", plot=h1_composition_time_plot)
ggsave(filename="plots/h1_average_composition_time_zoom.pdf", plot=h1_composition_time_plot_zoom)
